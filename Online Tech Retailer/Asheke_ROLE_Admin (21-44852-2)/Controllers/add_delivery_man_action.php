<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        if(empty($delivery_man_id)){
            $err_delivery_man_id="Delivery Man PIN Required";
        ...
        $sql=$conn->prepare("select * from delivery_men where `Delivery Man PIN`=?");
        $sql->bind_param("s",$delivery_man_id);
        $sql->execute();
        $data=$sql->get_result();
        ...
        if($has_error==false){
            ...
            $sql=$conn->prepare("insert into delivery_men values(null,?,?,?,?,?,?)");
            $rating=0;
            $sql->bind_param("ssssss",$name,$delivery_man_id,$email,$salary,$bonus,$rating);
            $sql->execute();
